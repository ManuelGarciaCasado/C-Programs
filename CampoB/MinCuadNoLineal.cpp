...
int main()
{
	...
	double anxm[n][m], amxm[m][m], bmx1[m], inclamb[m], incbeta[n], lambdamx1[m],
		suma, xnx1[n], ynx1[n], vectmx1[m],ain[m][m+1],xout[m];
	double* modlamb;
	...
	regresionlineal(xnx1, ynx1, A, B, coef_det, num_linea - 1);
	printf("Calculando parámetros A, B, C, D:\n\n");
	printf("                                      1/2  \n");
    printf("                  /              2   \\    \n");
	printf("y = A + B x +    |  1 + (C + D x)     |    \n");
    printf("                  \\                 /     \n");
	lambdamx1[0] = *A-sqrt(2);
	lambdamx1[1] = *B;
	lambdamx1[2] = 1;
	lambdamx1[3] = 0;
	//lambdamx1[4] = 1;
...
int norma(double x[n], int tam, double *modulo)
{
...
int regresionlineal(double x[n], double y[n], double* beta_0, double* beta_1, double *R2, int nfil)
{
	...
	for (int i = 0; i < nfil; i++)
	{
			...
			x_media += x[i] / nfil;
			y_media += y[i] / nfil;
			xy_media += x[i]* y[i] / nfil;
			xx_media += x[i]* x[i] / nfil;
			yy_media += y[i] * y[i] / nfil;
	}
	*beta_1 = (-xy_media-x_media*y_media) / (xx_media-x_media*x_media);
	*beta_0 = y_media - *beta_1 * x_media;

	coef_corr = sqrt(xx_media - x_media*x_media) / sqrt(yy_media - y_media*y_media)*(*beta_1);
	*R2 = coef_corr*coef_corr;
	return 0;
