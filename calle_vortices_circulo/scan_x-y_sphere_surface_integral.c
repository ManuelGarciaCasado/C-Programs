...
double Integrando2(int a, int b, int c, int d, double *xArray,
		double *yArray, double *uArray,
		double *vArray, double *pArray);
double Integrando3(int a, int b, int c, int d, double *xArray,
		double *yArray, double *uArray,
		double *vArray, double *pArray);
...
void main(int argc, char* argv[])
{
  ...
  double *pData = 0;
  double *dblBuffer = 0;
  double circunferencia1[100] = {0}, circunferencia2[100] = {0},circunferencia3[100] = {0};
  double campo1 = 0, campo2 = 0, campo3 = 0;
  double esfera1 = 0, esfera2 =0,esfera3 =0; 

  char Xchar[5],Ychar[5],Rchar[5],Tini[3], Tfin[3], Tchar[3];
  char Unamefile[10]={0},Vnamefile[10]={0},Pnamefile[10]={0};
  int  X,Y,R, ti,tf,t,Xini,Yini;
  ...
  if(argc < 4)
  {
   ...
   printf ("Center R radius (R <= 37): ");
  gets (Rchar);
  /*printf ("Begin from X  (R+2 < X < 440-R-2): ");
  gets (Xchar);
  printf ("and from Y (R+2 < Y < 82-R-2): ");
  gets (Ychar);*/
  R= atoi(Rchar);
  //Xini= atoi(Xchar);
  //Yini= atoi(Ychar);
  ...
  fprintf(Output,"R = %d\n", R);
  fprintf(Output,"X |Y	|Convexity Int.        | Minimum Int.\n");
  fclose(Output);
...
//  printf("x: %s = %+.20e\n", xval, atof(xval));
  ...
  for (X = R+3; X <= 440-R-3; X++){
  for (Y = R+3; Y <= 82-R-3; Y++){
  x = xData[X * 82 + Y];
  y = yData[X * 82 + Y];
  ...
  for(k = 1; k < R+1; k++){
	...
	for(i = 0; i < 440; i++){
		for(j = 0; j < 82; j++){
			...
			campo3 = Integrando3(i, j, X, Y, xData, yData, uData, vData, pData);
			circunferencia1[k] = circunferencia1[k] + campo1 * xinc;
			circunferencia2[k] = circunferencia2[k] + campo2 * xinc;
			circunferencia3[k] = circunferencia3[k] + campo3 * xinc;
  ...
  for(k = 0; k < 100; k++){
	...
	f = sqrt(pow(k,2) + pow(l,2));
		if ( f >= (double)R-0.5 && f <= (double)R+0.5 ){
			...
			esfera3 = esfera3 + circunferencia3[k] * zinc;
...
/*  printf("Integral de convexidad (>=0): %+.20e\n", esfera1);
  ...
  printf("Integral de divergencia: %+.20e\n", esfera3);*/
  //Output = fopen("OUTPUT", "a");
  fprintf(Output,"%d	%d	%+.10e	%+.10e	%+.10e\n", X, Y, esfera1, esfera2, esfera3);
  //fclose(Output);
...
double *yArray, double *uArray, double *vArray,double *pArray){
  ...
  v = vArray[a*col+b];
  p = pArray[a*col+b];
  dx=xArray[(a+1)*col+b] - xArray[a*col+b];
  dy=yArray[a*col+(b+1)] - yArray[a*col+b];
  dxu=(uArray[(a+1)*col+b] - uArray[(a-1)*col+b])/(2*dx);
  dxv=(vArray[(a+1)*col+b] - vArray[(a-1)*col+b])/(2*dx);
  dyu=(uArray[a*col+(b+1)] - uArray[a*col+(b-1)])/(2*dy);
  dyv=(vArray[a*col+(b+1)] - vArray[a*col+(b-1)])/(2*dy);
  dxxu=(uArray[(a+1)*col+b] - 2*uArray[a*col+b]
	   + uArray[(a-1)*col+b])/(dx*dx);
  dyyu=(uArray[a*col+(b+1)] - 2*uArray[a*col+b]
       + uArray[a*col+(b-1)])/(dx*dx);
  dxyu=(uArray[(a+1)*col+(b+1)] - uArray[(a-1)*col+(b+1)]
  	   - uArray[(a+1)*col+(b-1)] + uArray[(a-1)*col+(b-1)])/(4*dx*dy);
  dxxv=(vArray[(a+1)*col+b] - 2*vArray[a*col+b]
       + vArray[(a-1)*col+b])/(dx*dx);
  dyyv=(vArray[a*col+(b+1)] - 2*vArray[a*col+b]
       + vArray[a*col+(b-1)])/(dx*dx);
  dxyv=(vArray[(a+1)*col+(b+1)] - vArray[(a-1)*col+(b+1)]
	   - vArray[(a+1)*col+(b-1)] + vArray[(a-1)*col+(b-1)])/(4*dx*dy);
  dxxp=(pArray[(a+1)*col+b] - 2*pArray[a*col+b]
       + pArray[(a-1)*col+b])/(dx*dx);
  dyyp=(pArray[a*col+(b+1)] - 2*pArray[a*col+b]
       + pArray[a*col+(b-1)])/(dx*dx);
  dxyp=(pArray[(a+1)*col+(b+1)] - pArray[(a-1)*col+(b+1)]
	   - pArray[(a+1)*col+(b-1)] + pArray[(a-1)*col+(b-1)])/(4*dx*dy);
  dyxp=dxyp;
  dxxxu=(uArray[(a+2)*col+b] - 2*uArray[(a+1)*col+b]
	    + 2*uArray[(a-1)*col+b] - uArray[(a-2)*col+b])/(2*dx*dx*dx);
  dxxyu=(uArray[(a+1)*col+(b+1)] - 2*uArray[a*col+(b+1)]
	    + uArray[(a-1)*col+(b+1)]
		- uArray[(a+1)*col+(b-1)] + 2*uArray[a*col+(b-1)]
	    - uArray[(a-1)*col+(b-1)])/(2*dx*dx*dy);
  dyyyu=(uArray[a*col+(b+2)] - 2*uArray[a*col+(b+1)]
	    + 2*uArray[a*col+(b-1)] - uArray[a*col+(b-2)])/(2*dy*dy*dy);
  dyyxu=(uArray[(a+1)*col+(b+1)] - 2*uArray[(a+1)*col+b]
	    + uArray[(a+1)*col+(b-1)] - uArray[(a-1)*col+(b+1)]
	    + 2*uArray[(a-1)*col+b] - uArray[(a-1)*col+(b-1)])/(2*dy*dy*dx);
  dxxxv=(vArray[(a+2)*col+b] - 2*vArray[(a+1)*col+b]
		+ 2*vArray[(a-1)*col+b] - vArray[(a-2)*col+b])/(2*dx*dx*dx);
  dxxyv=(vArray[(a+1)*col+(b+1)] - 2*vArray[a*col+(b+1)]
	    + vArray[(a-1)*col+(b+1)] - vArray[(a+1)*col+(b-1)]
	    + 2*vArray[a*col+(b-1)] - vArray[(a-1)*col+(b-1)])/(2*dx*dx*dy);
  dyyyv=(vArray[a*col+(b+2)] - 2*vArray[a*col+(b+1)]
	    + 2*vArray[a*col+(b-1)] - vArray[a*col+(b-2)])/(2*dy*dy*dy);
  dyyxv=(vArray[(a+1)*col+(b+1)] - 2*vArray[(a+1)*col+b]
		+ vArray[(a+1)*col+(b-1)] - vArray[(a-1)*col+(b+1)]
		+ 2*vArray[(a-1)*col+b] - vArray[(a-1)*col+(b-1)])/(2*dy*dy*dx);
...
double *yArray, double *uArray, double *vArray, double *pArray){
  ...
  Valor = ((X-x)*u + (Y-y)*v)/sqrt(pow(X-x,2) + pow(Y-y,2));
